...
function formatTokenCount(theme: ThemeLike, usage: AggregatedUsage): string | null {
...
function formatBranch(theme: ThemeLike, branch: string | null, dirty: boolean): string | null {
  if (!branch) {
    return null;
  }
...
function buildFooterLine(theme: ThemeLike, width: number, left: string, rightSegments: FooterSegment[]): string {
  const dropOrder = ["status", "cost", "tokens", "thinking", "model"];
  const activeSegments = [...rightSegments];

  const getRight = () => joinSegments(theme, activeSegments.map((segment) => segment.text));
  const totalWidth = (leftText: string, rightText: string) =>
    visibleWidth(leftText) + (leftText && rightText ? 1 : 0) + visibleWidth(rightText);

  let right = getRight();

  while (totalWidth(left, right) > width) {
    const droppableKey = dropOrder.find((key) => activeSegments.some((segment) => segment.key === key && !segment.required));
    if (!droppableKey) {
      break;
    }
    const index = activeSegments.findIndex((segment) => segment.key === droppableKey && !segment.required);
    if (index === -1) {
      break;
    }
  ...
  if (totalWidth(fittedLeft, right) <= width) {
    const pad = " ".repeat(Math.max(1, width - visibleWidth(fittedLeft) - visibleWidth(right)));
  ...
  const fallbackLeftWidth = Math.max(0, Math.min(visibleWidth(fittedLeft), Math.floor(width * 0.35)));
  const fallbackLeft = truncateLeft(fittedLeft, fallbackLeftWidth);
  const fallbackRightWidth = Math.max(0, width - visibleWidth(fallbackLeft) - (fallbackLeft ? 1 : 0));
  ...
  const pad = " ".repeat(Math.max(1, width - visibleWidth(fallbackLeft) - visibleWidth(fallbackRight)));
    ...
    ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          const statusSegments: FooterSegment[] = [...footerData.getExtensionStatuses().values()].map((text) => ({
            key: "status",
            text,
          }));
          ...
          const rightSegments: FooterSegment[] = [
            ...
            ...statusSegments,
          ].filter((segment): segment is FooterSegment => Boolean(segment.text));
